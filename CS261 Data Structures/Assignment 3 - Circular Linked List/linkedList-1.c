...
static void addLinkBefore(struct LinkedList* list, struct Link* link, TYPE value)
{
    // FIXME: you must write this
    struct Link* add;
    add = (struct Link*) malloc(sizeof(struct Link));
...
{
    // FIXME: you must write this
    if (list != NULL && !linkedListIsEmpty(list))
    {
...
void linkedListPrint(struct LinkedList* list)
{
    ...
    if (list != NULL && !linkedListIsEmpty(list))
    {
        struct Link* newLink = list->frontSentinel->next;
        int i = 1;
        while (newLink != list->backSentinel){
            printf("Link #%d: %d\n", i, newLink->value);
