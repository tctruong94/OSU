...
void addHeap(DynArr *heap, TYPE node)
{
  	...
  	while(currentNode != 0) 
  	{
  		...
  		if(compare(getDynArr(heap, currentNode), getDynArr(heap, parentNode)) == -1) 
  		{
  			swapDynArr(heap, currentNode, parentNode);
...
void _adjustHeap(DynArr *heap, int max, int pos)
{
	...
	assert(pos <= max);
  	...
  	{																				/*two child*/
  		...
  		{
  			swapDynArr(heap, pos, smallerChild);
